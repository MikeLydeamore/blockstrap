...
> Assumption: The package name in `DESCRIPTION` is `blockstrap` (repository folder is `flockstrapr`). This README uses the `blockstrap` name; adjust if the package is renamed.
...
- `...`: passed to base `sample()` allowing e.g. `set.seed()` beforehand or other sampling tweaks.
